...
export class DatabaseService {
  ...
  static async getPurchases() {
    ...
    try {
      // Check if purchases table exists by attempting a simple query
      ...
      if (tableCheckError && tableCheckError.code === 'PGRST205') {
        console.warn('Purchases table does not exist yet. Please apply the migration.');
        return [];
      }
  ...
  static async getSales() {
    ...
    try {
      // Check if sales table exists by attempting a simple query
      const { error: tableCheckError } = await supabase
        .from('sales')
        .select('id')
        .limit(1);
      ...
      if (tableCheckError && tableCheckError.code === 'PGRST205') {
        console.warn('Sales table does not exist yet. Please apply the migration.');
        return [];
      }
